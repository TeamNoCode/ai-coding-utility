// ====================================================
//  VERSION: codr-src
//  FILE: src/components/codr-src-ChatInterface.tsx
// ====================================================
...
import { startChatSession } from '../services/codr-src-geminiService';
import { supabase, supabaseEnabled } from '../services/codr-src-supabaseClient';
import SidePanel from './codr-src-SettingsPanel';
import MessageComponent, { parseContent, ParsedCodePart } from './codr-src-Message';
import ThinkingMessage from './codr-src-ThinkingMessage';
import CodeBlock from './codr-src-CodeBlock';
import { SendIcon, ChevronDoubleRightIcon, CloseIcon, SparklesIcon, BrainCircuitIcon, PreviewIcon, UndoIcon, RedoIcon, PlusIcon, XCircleIcon } from './codr-src-icons';
import { DEFAULT_LANGUAGE, DEFAULT_TEMPERATURE, WELCOME_PROMPTS_LIST } from '../codr-src-constants';
...
const FocusPanel: React.FC<FocusPanelProps> = ({ messages, focusedMessageId, focusMode, onClose, onUndo, canUndo, onRedo, canRedo }) => {
    ...
    }, [focusedMessageId, messages, focusedMessage]);
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ session }) => {
  ...
  useEffect(() => {
    // Shuffle prompts on initial load
    const shuffled = [...WELCOME_PROMPTS_LIST].sort(() => 0.5 - Math.random());
    setWelcomePrompts(shuffled.slice(0, 4));
  }, []);
  ...
  const handleSendMessage = async (messageContent?: string, imageContent?: string | null) => {
    ...
    try {
        const chatSession = startChatSession(language, temperature, historyForAI, customPrompt);
        ...
        if (session && supabase && currentChatId) {
  ...
  const handleBookmark = () => {
    if (!session && supabaseEnabled) {
        ...
        alert("User accounts are not configured for this application. Please set up Supabase credentials to save chats.");
    }
  };
