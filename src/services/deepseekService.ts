...
if (!apiKey) {
    console.warn("VITE_DEEPSEEK_API_KEY environment variable not set. The built-in key will not be available.");
}

const roleToDeepSeek = (role: MessageRole): 'user' | 'assistant' | 'system' => {
...
const createDeepSeekChatSession = (options: ChatOptions, history: Message[], systemInstruction: string) => {
    const { temperature, llmSettings } = options;

    const formattedHistory = [
        { role: 'system', content: systemInstruction },
        ...history.map(msg => ({
            role: roleToDeepSeek(msg.role),
            content: msg.content || '',
        }))
...
export const startChatSession = (
    ...
    return createDeepSeekChatSession(options, history, systemInstruction);
};
